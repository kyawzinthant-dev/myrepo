...
def get_sa(update, context):
    ...
    token_file = open(os.path.join(d+os.sep+"telegram_gcloner"+os.sep+"token.json"),"r")
    cid = open(os.path.join(d+os.sep+"telegram_gcloner"+os.sep+"client_id.txt"),"r")
    csecret = open(os.path.join(d+os.sep+"telegram_gcloner"+os.sep+"client_secret.txt"),"r")
    contents = token_file.read()
    client_id = cid.read()
    client_secret = csecret.read()
    #token = ast.literal_eval(contents)
    #token_file.close()

    config_file.add_section('src')
    config_file.set('src', 'type', 'drive')
    config_file.set('src', 'scope', 'drive')
    config_file.set('src', 'client_id', client_id)
    config_file.set('src', 'client_secret', client_secret)
    config_file.set('src', 'token', contents)
    config_file.set('src', 'root_folder_id', 'root')
